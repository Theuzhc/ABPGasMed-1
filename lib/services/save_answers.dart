...
class AnswerRepository {
  Future<Database> _getDatabase() async {
    return openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
            'CREATE TABLE answered(id INTEGER PRIMARY KEY, answer TEXT),');
      },
      version: 1,
    );
  }

  Future create(AnswerRepository answers) async {
    try {
  ...
  Future<List<AnswerRepository>> getQuestion() async {
    try {
      final Database db = await _getDatabase();
